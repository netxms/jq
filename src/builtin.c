...
static block gen_last_1(void) {
  ...
  block init = BLOCK_6(gen_op_simple(DUP),
                     gen_const(jv_null()),
                     last_var,
                     gen_op_simple(DUP),
                     gen_const(jv_true()),
                     is_empty_var);
  block call_arg = BLOCK_6(gen_call("arg", gen_noop()),
                         gen_op_simple(DUP),
  ...
  return BLOCK_4(init,
               ...
               call_arg,
               BLOCK_4(gen_op_bound(LOADVN, is_empty_var),
...
static block bind_bytecoded_builtins(block b) {
  block builtins = gen_noop();
  {
    ...
    for (unsigned i=0; i<sizeof(builtin_defs)/sizeof(builtin_defs[0]); i++) {
      builtins = BLOCK_2(builtins, gen_function(builtin_defs[i].name, gen_noop(),
                                              builtin_defs[i].code));
    }
  }
  {
    struct bytecoded_builtin builtin_def_1arg[] = {
      {"path", BLOCK_3(gen_op_simple(PATH_BEGIN),
                     gen_call("arg", gen_noop()),
                     gen_op_simple(PATH_END))},
      {"last", gen_last_1()},
    };
    for (unsigned i=0; i<sizeof(builtin_def_1arg)/sizeof(builtin_def_1arg[0]); i++) {
      builtins = BLOCK_2(builtins, gen_function(builtin_def_1arg[i].name,
                                              gen_param("arg"),
  ...
  }
  {
    ...
    block range = BLOCK_8(gen_op_simple(DUP),
    ...
    builtins = BLOCK_2(builtins, gen_function("range",
                                            BLOCK_2(gen_param("start"), gen_param("end")),
                                            range));
  }
  return BLOCK_2(builtins, b);
}
...
static block gen_builtin_list(block builtins) {
  ...
  return BLOCK_2(builtins, gen_function("builtins", gen_noop(), gen_const(list)));
}
